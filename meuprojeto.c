...
void instrucao(){
    printf("\n INSTRUÇÃO \n");
    printf("Existem teclas númericas de 1 a 4, navegue digitando elas no espaço necessário.");
    system("pause");
    system("cls");
    menu();
}
...
printf("%dº - %s",x+1,listafilme[x].titulo);
    }else{
...
void listar1(){
    for(y=0;y<conty;y++){
        if(listacliente[y].id!=0){
            printf("%dº - %s",y+1,listacliente[y].nome);
            }else{
